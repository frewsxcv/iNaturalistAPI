...
TaxaController.replaceInactiveTaxaCounts = async ( counts, options = { } ) => {
  ...
  const countsObject = _.fromPairs( _.map( counts, c => [c.taxon_id, _.omit( c, "taxon_id" )] ) );
      ...
      if ( _.isEmpty( h._source.current_synonymous_taxon_ids ) ) {
        ...
        _.each( h._source.current_synonymous_taxon_ids, synTaxonID => {
          ...
          if (
            ...
            || countsObject[synTaxonID].count < countsObject[h._source.id].count
          ) {
  ...
  const updatedCounts = _.map( countsObject, ( count, taxonID ) => (
    Object.assign( { taxon_id: Number( taxonID ) }, count )
  ) );
    ...
    if ( req.suggestParams.source === "visual" ) {
      ...
      if ( obs[0].taxon ) {
        if ( obs[0].taxon.rank_level <= 10 ) {
          req.suggestParams.taxon_id = obs[0].taxon.ancestor_ids[obs[0].taxon.ancestor_ids.length - 2];
        } else {
...
TaxaController.getChecklistSuggestions = async req => {
  ...
  if ( !req.suggestParams.limit && req.suggestParams.limit !== 0 ) {
    ...
    limit = parseInt( req.suggestParams.limit, 0 );
    if ( limit > 500 ) {
      limit = 500;
    }
  }
...
TaxaController.getMisidentificationSuggestions = async req => {
  ...
  const response = await ObservationsController.similarSpecies(
    Object.assign( {}, req, { query: req.suggestParams } )
  );
  return {
    comprehensiveness: {},
...
TaxaController.getObservationSuggestions = async req => {
  ...
  const obsQuery = Object.assign( {}, req.suggestParams );
  ...
  const obsReq = Object.assign( {}, req, { query: obsQuery } );
