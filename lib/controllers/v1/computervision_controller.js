...
const ComputervisionController = class ComputervisionController {
  ...
  static async scoreImageURL( req, options = { } ) {
    return new Promise( ( resolve, reject ) => {
      if ( !req.userSession && !req.applicationSession ) {
        reject( util.httpError( 401, "Unauthorized" ) );
        return;
      }
      const photoURL = req.query.image_url;
      if ( !photoURL ) {
        void reject( util.httpError( 422, "No scorable photo" ) );
        return;
      }
      // download the JPG
  ...
  static async scoreImageUpload( uploadPath, req ) {
    InaturalistAPI.setPerPage( req, { default: 50, max: 100 } );
    const scores = await ComputervisionController.scoreImagePath( uploadPath, req );
    if ( _.isEmpty( scores ) ) {
      return InaturalistAPI.basicResponse( req );
    }
    const response = await TaxaController.speciesCountsResponse( req, scores, { } );
    const commonAncestor = _.find( response.results, r => r.count === -100 );
    if ( commonAncestor ) {
      response.common_ancestor = {
        taxon: commonAncestor.taxon
      };
    }
    response.results = _.filter( response.results, r => r.count !== -100 );
    _.each( response.results, r => {
      r.combined_score = r.count;
      delete r.count;
    } );

    // if human is the most likely result, then only return humans
    ...
    // consider human exclusion if there is no common ancestor, or the common
    // ancestor is humans
    if (
      !commonAncestor
    ...
    response.total_results = _.size( response.results );
    response.per_page = response.total_results;
    return response;
  }
  ...
  static async scoreImagePath( uploadPath, req ) {
    ...
    try {
      ...
      // TODO: why can't this be caught upstream
      // throw util.httpError( 500, "Error scoring image" );
      return [];
    }
    const counts = _.map( json.top_combined, taxonScore => ( {
      taxon_id: taxonScore.id,
      count: taxonScore.combined_score,
      frequency_score: ( taxonScore.geo_score > 10 ) ? taxonScore.geo_score : 0,
      vision_score: taxonScore.vision_score
    } ) );
    // counts = _.filter( counts, c => c.count >= 1 );
    if ( json.common_ancestor ) {
      counts.push( {
        taxon_id: json.common_ancestor.id,
        count: -100
      } );
    }
  ...
  static async scoreImagePathFrequencies( uploadPath, req ) {
    if ( req.inat && req.inat.visionCacheKey ) {
      const cachedScores = FileCache.cacheExists( req.inat.visionCacheKey );
      if ( cachedScores ) {
        return JSON.parse( cachedScores );
  ...
  static async scoreImageUploadFrequencies( uploadPath, req ) {
    InaturalistAPI.setPerPage( req, { default: 10, max: 100 } );
    const imageScores = await ComputervisionController.scoreImagePathFrequencies( uploadPath, req );
